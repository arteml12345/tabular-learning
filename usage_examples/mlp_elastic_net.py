"""Usage example: MLP embedding + Elastic Net classical model.
...
hybrid (NN + Elastic Net) are evaluated on the held-out OOS half, so
...
# =====================================================================
# Embedding model search space: MLP
# =====================================================================
...
# =====================================================================
# Classical model search space: Elastic Net
# =====================================================================

# Classification uses SGDClassifier (log_loss + elasticnet penalty);
# regression uses sklearn ElasticNet. They share alpha/l1_ratio names
# but have different valid ranges, so we define separate spaces.

# Classification search space (SGDClassifier):
...
# Classification fixed params:
classification_fixed_params = {
    # Loss function for logistic regression via SGD.
    "loss": "log_loss",
    # Penalty type: elastic net (combined L1 + L2).
    "penalty": "elasticnet",
    # Maximum iterations for convergence.
    "max_iter": 2000,
}

# Regression search space (sklearn ElasticNet):
regression_search_space = {
    # Regularization strength.
    "alpha": hp.loguniform("alpha", np.log(1e-4), np.log(1e2)),

    # Mixing ratio between L1 and L2.
    "l1_ratio": hp.uniform("l1_ratio", 0.0, 1.0),
}

# Regression fixed params:
regression_fixed_params = {
    # Maximum iterations for convergence.
    "max_iter": 1000,
}
...
def build_embedding_step():
    """Build the EmbeddingStepConfig for MLP."""
...
def build_classical_step(task="binary"):
    """Build the ClassicalStepConfig for Elastic Net."""
    if task == "regression":
        return ClassicalStepConfig(
            model_type="elastic_net",
            model_class=None,
            search_space=regression_search_space,
            fixed_params=regression_fixed_params,
    ...
    return ClassicalStepConfig(
        ...
        fixed_params=classification_fixed_params,
        supports_early_stopping=False,
    )
...
def run_binary_example():
    print("=" * 70)
    print("EXAMPLE 1: Binary classification -- MLP + Elastic Net")
    print("=" * 70)
...
def run_multiclass_example():
    print("\n" + "=" * 70)
    print("EXAMPLE 2: Multiclass classification (4 classes) -- MLP + Elastic Net")
    print("=" * 70)
...
def run_regression_example():
    print("\n" + "=" * 70)
    print("EXAMPLE 3: Regression -- MLP + Elastic Net")
    print("=" * 70)
