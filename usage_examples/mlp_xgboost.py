"""Usage example: MLP embedding + XGBoost classical model.
...
# =====================================================================
# Shared search spaces (custom, fully explicit)
# =====================================================================

# Custom MLP embedding search space.
# Every HP is set explicitly; nothing is left to the built-in default.
mlp_search_space = {
    # Dimensionality of the embedding layer output.
    # Determines the size of the vector passed to the classical model.
    ...
    "width": hp.quniform("width", 128, 1024, 64),
    ...
    # Mini-batch size for embedding model training.
    "batch_size": hp.choice("batch_size", [256, 512, 1024]),
}

# Fixed MLP params that are NOT tuned during HPO:
mlp_fixed_params = {
    # Which hidden layer to extract embeddings from.
    # -1 = last hidden layer (before the prediction head).
    "embedding_layer": -1,
}

# Custom XGBoost search space.
# Covers all major XGBoost hyperparameters.
...
def build_embedding_step():
    """Build the EmbeddingStepConfig with all parameters explicit."""
    return EmbeddingStepConfig(
        # Which embedding architecture to use.
        # "mlp" = from-scratch MLP (Mode A).
        # Also: "tab_transformer", "tabnet", "gate", "category_embedding"
        model_type="mlp",

        # Custom Hyperopt search space for MLP hyperparameters.
        # None = use built-in default space.
        search_space=mlp_search_space,

        # Fixed params merged with HPO-sampled params (not tuned).
        # None = no fixed overrides.
        fixed_params=mlp_fixed_params,
    )


def build_classical_step():
    """Build the ClassicalStepConfig with all parameters explicit."""
    return ClassicalStepConfig(
        # Built-in model name from the registry.
        # Options: "logistic_regression", "ridge", "elastic_net",
        #   "random_forest", "extra_trees", "xgboost", "lightgbm", "catboost"
        # Use "custom" + model_class for user-provided sklearn-compatible models.
        model_type="xgboost",

        # Only used when model_type="custom". Ignored for built-in models.
        model_class=None,

        # Custom Hyperopt search space for XGBoost hyperparameters.
        # None = use built-in default space for xgboost.
        search_space=xgboost_search_space,

        # Fixed params passed to XGBoost constructor (not tuned).
        # None = no fixed overrides.
        fixed_params=xgboost_fixed_params,

        # Whether to use early stopping with eval_set during training.
        # None = auto-detect (True for xgboost/lightgbm/catboost).
        # Set explicitly to True here.
...
def run_binary_example():
    print("=" * 70)
    print("EXAMPLE 1: Binary classification -- MLP + XGBoost")
    print("=" * 70)
    ...
    classical_step = build_classical_step()
...
def run_multiclass_example():
    print("\n" + "=" * 70)
    print("EXAMPLE 2: Multiclass classification (4 classes) -- MLP + XGBoost")
    print("=" * 70)
...
def run_regression_example():
    print("\n" + "=" * 70)
    print("EXAMPLE 3: Regression -- MLP + XGBoost")
    print("=" * 70)
