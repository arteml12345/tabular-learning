"""Usage example: GATE embedding + Elastic Net classical model.
...
# =====================================================================
# Embedding model search space: GATE
# =====================================================================

# Custom GATE (Gated Additive Tree Ensemble) embedding search space.
gate_search_space = {
    # Number of GFLU (Gated Feature Learning Unit) stages.
    "gflu_stages": hp.quniform("gflu_stages", 2, 10, 1),

    # Depth of the soft decision trees.
    "tree_depth": hp.quniform("tree_depth", 3, 6, 1),

    # Number of soft decision trees in the ensemble.
    "num_trees": hp.quniform("num_trees", 5, 30, 5),

    # Activation for binning in the tree leaves.
    "binning_activation": hp.choice("binning_activation", ["entmoid", "sparsemoid"]),

    # Feature mask function controlling feature selection.
...
# No fixed params needed for GATE.
gate_fixed_params = None
...
def build_embedding_step():
    """Build the EmbeddingStepConfig for GATE."""
    return EmbeddingStepConfig(
        # "gate" = pytorch-tabular GATE (Mode B).
        model_type="gate",

        search_space=gate_search_space,
        fixed_params=gate_fixed_params,
    )
...
def run_binary_example():
    print("=" * 70)
    print("EXAMPLE 1: Binary classification -- GATE + Elastic Net")
    print("=" * 70)
...
def run_multiclass_example():
    print("\n" + "=" * 70)
    print("EXAMPLE 2: Multiclass classification (4 classes) -- GATE + Elastic Net")
    print("=" * 70)
...
def run_regression_example():
    print("\n" + "=" * 70)
    print("EXAMPLE 3: Regression -- GATE + Elastic Net")
    print("=" * 70)
